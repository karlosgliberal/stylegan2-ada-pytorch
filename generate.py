...
def circular_interpolation(radius, latents_persistent, latents_interpolate):
    ...
    latents_axis_x = (latents_a - latents_b).flatten() / \
        linalg.norm(latents_a - latents_b)
    latents_axis_y = (latents_a - latents_c).flatten() / \
        linalg.norm(latents_a - latents_c)
...
def interpolate(G, device, projected_w, seeds, random_seed, space, truncation_psi, label, frames, noise_mode, outdir, interpolation, easing, diameter, start=None, stop=None):
    if(interpolation == 'noiseloop' or interpolation == 'circularloop'):
        if seeds is not None:
            print(
                f'Warning: interpolation type: "{interpolation}" doesn’t support set seeds.')
    ...
    # generate frames
    images(G, device, points, space, truncation_psi,
...
def generate_images(
    ...
    with dnnlib.util.open_url(network_pkl) as f:
        ...
        G = legacy.load_network_pkl(
            f, custom=custom, **G_kwargs)['G_ema'].to(device)  # type: ignore
